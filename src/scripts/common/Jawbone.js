export class Jawbone  {
    ...
    create = (state, movie, e)  => {
        let section, jawbone, clone,
        trailers, reviews, similarMovies, navbar,
        nodesInBetween, nodeToAppendTo
        ;
        ...
        clone = this.insertAfter(jawbone.cloneNode(true), nodeToAppendTo);
        ...
        clone.querySelector('.js-genres').textContent = this.getGenres(movie.genres);
        ...
        if (movie.similar.results.length === 0) {
            ...
            Array.from(similarMovies).map((similar, i) => {
        ...
        Array.from(navbar.querySelectorAll('span')).map((navItem) => {
            navItem.addEventListener('click',  (e) => {
                this.showTab(clone, navbar, e);
            });
        });

        // Close button and remove active states from jawbone and parent movie card
        clone.querySelector('.js-jawbone-close').addEventListener('click', () => {
